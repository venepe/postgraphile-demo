...
export class PackCreatePage extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    const { history, match: { params: { babelId } }, classes } = this.props;
    const { packId } = this.state;
    let packComponent = null;
    if (this.state.packId) {
      packComponent = (<PackDetail className={classes.packContainer} packId={this.state.packId} />);
    }
    return (
    <section>
      <Query
...
export default withStyles(styles)(PackCreatePage);
